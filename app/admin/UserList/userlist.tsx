...
export default function InvestorList() {
  ...
  const confirmPermissionChange = async () => {
    ...
    const { userId, permissionType, permissionValue, userName } = pendingPermission
    setConfirmDialogOpen(false)

    try {
      const token = localStorage.getItem('token')
      if (!token) return toast.error('Authorization token missing.')
      ...
      if (updateUserPermission.fulfilled.match(resultAction)) {
        toast.success(
          `${permissionType} ${permissionValue ? 'enabled' : 'disabled'} for ${userName}`,
        )
        dispatch(
          fetchInvestorList({
            page: currentPage,
        ...
        toast.error(`Failed to update ${permissionType} for ${userName}`)
      }
    } catch (error) {
      console.error('Error updating permission:', error)
      toast.error(`Error while updating ${permissionType} for ${userName}`)
    }
  }
  ...
  const [selectedWalletKind, setSelectedWalletKind] = useState('');
  ...
  const formatWalletLabel = (id: string) => {
    return id
      .replace(/_/g, " ")
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-500 via-purple-600 to-purple-700 p-6">
      <div className="container mx-auto max-w-7xl">
                          ...
                          <td className="px-6 py-4">
                            <DropdownMenu>
                              ...
                              >
                                ...
                                {dropDownOptions?.wallet_kinds?.map((wallet: any) => (
                                  <DropdownMenuItem
                                    key={wallet.id}                       // e.g. "income_wallet"
                                    onClick={() => {
                                      dispatch(fetchInvestorDetails(investor.user_id));

                                      setWalletUserId(investor.user_id);
                                      setSelectedWalletId(wallet.id);     // "income_wallet"
                                      setSelectedWalletKind(wallet.value);// "IncomeWallet"
                                      setDialogOpen(true);
                                    }}
                                  >
                                    <Wallet className="w-4 h-4 mr-2" />
                                    {formatWalletLabel(wallet.id)}
                                  </DropdownMenuItem>
        ...
        <WalletManipulationDialog
          ...
          userId={walletUserId}
          walletId={selectedWalletId}      // "income_wallet"
          walletKind={selectedWalletKind}  // "IncomeWallet"
        />
        ...
        <Dialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
          <DialogContent className="sm:max-w-md bg-white shadow-xl rounded-2xl">
            <DialogHeader>
              ...
              {pendingPermission && (
                <p className="text-gray-700">
                  ...
                  <span className="font-semibold text-blue-600">
                    {pendingPermission.permissionValue ? 'enable' : 'disable'}
                  </span>{' '}
                  the{' '}
