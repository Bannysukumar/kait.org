...
import {
  initiateTransferVpayToIncome,
  resetTransferState,
} from '@/store/slices/user/vpayToIncomeTransfer'
import {
  transferWalletOtp,
  resetOtpState,
...
const VpayToIncomeTransferForm = () => {
  ...
  const handleRequestOtp = async () => {
    ...
    const maxWithdraw = balances?.VpayVoucher?.max_allowed_to_withdraw ?? 0
    if (amountNumber > maxWithdraw) {
      toast.error(`Insufficient Vpay Wallet balance. Max allowed to withdraw is ${maxWithdraw.toLocaleString()}`)
      return
    }
  ...
  useEffect(() => {
    dispatch(fetchWalletBalance('VpayVoucher'))
  }, [dispatch])
    ...
    <div className="pt-5 bg-gradient-to-r from-blue-500 to-purple-700 pb-10 mx-auto px-4 sm:px-6 lg:px-8">
      <div className="container m-auto">
        <h1 className="flex items-center text-2xl font-bold mb-6 mt-5">
          <ArrowRightLeft className="mr-2" /> Transfer Vpay Wallet
        </h1>
        ...
        <div className="bg-white rounded-lg shadow-lg p-4">
          <h2 className="text-lg p-2 rounded-md font-semibold w-fit bg-gradient-to-r from-blue-500 to-purple-700 text-white flex items-center">
            ...
            <Image
              ...
              className="ml-2 mr-1"
            />
            {Number(balances?.VpayVoucher?.total ?? 0).toLocaleString()}
          </h2>
          <span className="text-[12px] p-2 rounded-md font-semibold mb-2 w-fit  text-black flex items-center">
            ...
            />
            {Number(
              balances?.VpayVoucher?.max_allowed_to_withdraw ?? 0,
            ).toLocaleString()}
          </span>
          ...
          <div className="flex flex-col md:flex-row gap-6">
              ...
              <Dialog
                ...
                slotProps={{
                  paper: {
                    sx: {
                      borderRadius: "28px",
                      background: "#efe7f7", // same as your styled dialog
                      paddingTop: "10px",
                      paddingBottom: "10px",
                    },
                  },
                ...
                >
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    <TextField
                      ...
                      onChange={handleChange}
                      inputProps={{ style: { paddingLeft: "30px" } }}
                      InputLabelProps={{
                        sx: { color: "#5e3db5 !important", fontWeight: 600, marginLeft: "30px" },
                      }}
                      sx={{
                        "& .MuiInput-underline:before": { borderBottomColor: "#b39ddb" },
                        "& .MuiInput-underline:after": { borderBottomColor: "#5e3db5" },
            ...
            <div className="w-full md:w-1/3 p-4 rounded-xl bg-white shadow-sm border border-gray-200">
              ...
              <div className="space-y-3 text-[12px] text-gray-700">
                {[
                  'Vpay Wallet can be transferred to your Income Wallet.',
                  'Maintain a minimum of 0 in your Vpay Wallet, above which can be transferred.',
                  '2% Admin Charges',
                  'You can only transfer to your own Income Wallet.',
                ].map((rule, idx) => (
                  <div
                    key={idx}
...
export default VpayToIncomeTransferForm