...
import { fixedRosWalletBalance } from '@/store/slices/user/walletWithdrawal/fixedRosWalletbalance'
import {
  withdrawFixedRos,
  resetWithdrawState,
} from '@/store/slices/user/walletWithdrawal/fixedRosWithdrawForm'
import {
  transferWalletOtp,
  resetOtpState,
} from '@/store/slices/user/TransferWalletOtpSlice'
import { fixedRosWithdrawSummary } from '@/store/slices/user/walletWithdrawal/fixedRosWithdrawalSummary'
...
export default function RosWalletSummary() {
  ...
  const rosState = useAppSelector((state) => state.FixedRoswithdraw)
  const withdrawState = useAppSelector((state) => state.fixedroswithdrawForm)
  ...
  const {
    ...
    error: summaryError,
  } = useAppSelector((state) => state.fixedRosWalletSummary)
  ...
  useEffect(() => {
    dispatch(fixedRosWalletBalance())
  ...
  const handleWithdrawSubmit = (e: React.FormEvent) => {
    ...
    dispatch(withdrawFixedRos(formData))
  }
        ...
        <Box className="space-y-8 max-w-2xl w-full">
          <Box className="p-4 shadow rounded bg-white">
            ...
            <div className="bg-gradient-to-r from-blue-500 to-purple-700 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center">
              ...
              <div className="p-5 text-white">
                ...
                <div className="flex gap-3 flex-wrap text-lg sm:text-2xl mt-1">
                  <Typography
                    className="flex border-2 rounded-2xl border-white p-2 gap-2 items-center"
                    component="div"
                  >
            ...
            <Typography className=' flex ' style={{ marginLeft: "10px" }}>
              Max Withdrawal Limit 40,000 <br/>Minimum of 100 KAIT to be maintain in Fixed ROS Wallet
            </Typography>
            {/* Withdraw Form */}
            <Box
              component="form"
        ...
        <Box className="space-y-4 bg-white p-4 sm:p-6 rounded shadow max-w-full overflow-x-auto">
          ...
          {!summaryLoading && (
            <TableContainer component={Paper} className="shadow overflow-x-auto">
              <Table size="small" aria-label="Fixed ROS Withdrawal Table">
                <TableHead>
